...
-- 24/05/2022 - Robert - Incluida coluna LOTE_FILIAL_DESTINO (GLPI 11980)
--
...
AS
	...
	WITH C AS (
		...
		SELECT '2' AS TIPO_REG
			...
			-- LOTE NA FILIAL ORIGEM EH BUSCADO NA TABELA SD2 (NF SAIDA NA FILIAL ORIGEM)
			-- CONFORME O TIPO DE ENTRADA (USA OU NAO CLIENTE), TAMBEM A SAIDA NA
			-- FILIAL DE ORIGEM DEVE TER USADO OU NAO FORNECEDOR.
			-- BUSCA USANDO 'TOP 1' POR QUE PODE OCORRER A TRANSFERENCIA DE MAIS DE UM LOTE
			-- DO MESMO PRODUTO NA MESMA NOTA. ORDENA PELA MENOR DIFERENCA ENTRE O ITEM DAS
			-- NOTAS DE ORIGEM E DE DESTINO, POIS POR VOLTA DO ANO 2020 O SISTEMA COMECOU A
			-- EXIGIR QUE NOTAS DE TRANSF.ENTRE FILIAIS SEJAM DIGITADAS NA MESMA SEQUENCIA.
			, CASE WHEN D1_TIPO IN ('B', 'D') AND A1_FILTRF != ''
				THEN (SELECT TOP 1 D2_LOTECTL
						...
						ORDER BY abs (CAST (D2_ITEM AS INT) - CAST (D1_ITEM AS INT))  -- MAIS RECENTEMENTE O SISTEMA COMECOU A OBRIGAR MANTER A MESMA ORDENACAO
					)
		...
		*/
		...
		SELECT '2' AS TIPO_REG,
			...
			-- LOTE GERADO NA FILIAL DESTINO EH BUSCADO NA TABELA SD1 (NF ENTRADA NA FILIAL DESTINO)
			-- CONFORME O TIPO DE SAIDA NA ORIGEM (USA OU NAO FORNECEDOR), TAMBEM A ENTRADA
			-- NA FILIAL DESTINO VAI USAR OU NAO CLIENTE.
			-- BUSCA USANDO 'TOP 1' POR QUE PODE OCORRER A TRANSFERENCIA DE MAIS DE UM LOTE
			-- DO MESMO PRODUTO NA MESMA NOTA. ORDENA PELA MENOR DIFERENCA ENTRE O ITEM DAS
			-- NOTAS DE ORIGEM E DE DESTINO, POIS POR VOLTA DO ANO 2020 O SISTEMA COMECOU A
			-- EXIGIR QUE NOTAS DE TRANSF.ENTRE FILIAIS SEJAM DIGITADAS NA MESMA SEQUENCIA.
			-- PARA ENCONTRAR O FORNECEDOR CORRETO NA FILIAL DESTINO, APROVEITA O RELACIONAMENTO
			-- EXISTENTE NOS CAMPOS PADRAO A1_FILTRF E A2_FILTRF.
			, CASE WHEN D2_TIPO IN ('B', 'D') AND A2_FILTRF != ''  -- NOTA 'USA FORNECEDOR'
				THEN (SELECT TOP 1 D1_LOTECTL
						FROM SD1010 SD1
							,SA1010 SA1  -- NOTA DEVE SER TIPO 'B/D' NA FILIAL DESTINO
						WHERE SD1.D_E_L_E_T_ = ''
						AND SD1.D1_FILIAL  = SA2.A2_FILTRF
						AND SD1.D1_DOC     = SD2.D2_DOC
						AND SD1.D1_SERIE   = SD2.D2_SERIE
						AND SD1.D1_COD     = SD2.D2_COD
						AND SD1.D1_QUANT   = SD2.D2_QUANT
						AND SD1.D1_FORNECE = SA1.A1_COD
						AND SD1.D1_LOJA    = SA1.A1_LOJA
						AND SA1.D_E_L_E_T_ = ''
						AND SA1.A1_FILIAL  = '  '
						AND SA1.A1_FILTRF  = SD2.D2_FILIAL
						ORDER BY abs (CAST (D1_ITEM AS INT) - CAST (D2_ITEM AS INT))
					)
				ELSE CASE WHEN D2_TIPO NOT IN ('B', 'D') AND A1_FILTRF != ''  -- NOTA 'USA CLIENTE'
						THEN (SELECT TOP 1 D1_LOTECTL
								FROM SD1010 SD1
									,SA2010 SA2  -- NOTA DEVE SER TIPO 'N' NA FILIAL DESTINO
								WHERE SD1.D_E_L_E_T_ = ''
								AND SD1.D1_FILIAL  = SA1.A1_FILTRF
								AND SD1.D1_DOC     = SD2.D2_DOC
								AND SD1.D1_SERIE   = SD2.D2_SERIE
								AND SD1.D1_COD     = SD2.D2_COD
								AND SD1.D1_QUANT   = SD2.D2_QUANT
								...
								AND SA2.D_E_L_E_T_ = ''
								AND SA2.A2_FILIAL  = '  '
								AND SA2.A2_FILTRF  = SD2.D2_FILIAL
								ORDER BY abs (CAST (D1_ITEM AS INT) - CAST (D2_ITEM AS INT))
							)
						ELSE ''
						END
				END AS LOTE_FILIAL_DESTINO
