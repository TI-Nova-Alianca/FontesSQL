...
ALTER FUNCTION VA_FDRE_PROJETADO
...
BEGIN
	...
	, NIVEIS AS (SELECT 1  AS ORDEM, '01.(+)INGRESSOS E RECEITAS'      AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 1
	   UNION ALL SELECT 2  AS ORDEM, '02.(-)DEVOLUCOES'                AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 2
	   UNION ALL SELECT 4  AS ORDEM, '04.(-)IMPOSTOS'                  AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 4
	   UNION ALL SELECT 6  AS ORDEM, '06.(-)CPV'                       AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 6
	   UNION ALL SELECT 8  AS ORDEM, '08.(=)SOBRA OPERACIONAL BRUTA'   AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 8
	   UNION ALL SELECT 9  AS ORDEM, '09.(-)DESPESAS OPERACIONAIS'     AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 9
	   UNION ALL SELECT 10 AS ORDEM, '10.(-)DESPESAS ADMINISTRATIVAS'  AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 10
	   UNION ALL SELECT 11 AS ORDEM, '11.(-)DESPESAS TRIBUTARIAS'      AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 11
	   UNION ALL SELECT 12 AS ORDEM, '12.(-)DESPESAS COM VENDAS'       AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 12
	   UNION ALL SELECT 15 AS ORDEM, '15.(+)RESULTADO FINANCEIRO'      AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 15
	   UNION ALL SELECT 16 AS ORDEM, '16.(+)RESULTADO NAO OPERACIONAL' AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 16
	   UNION ALL SELECT 18 AS ORDEM, '18.INFORMATIVOS'                 AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 18
	)
	...
	SELECT NIVEIS.CTS_CODPLA AS CODPLA,
		   ...
		   ISNULL (NIVEIS.GRUPO,    '18.INFORMATIVOS') AS GRUPO,
		   ISNULL (NIVEIS.SUBGRUPO, 'SEM CLASSIFICACAO') AS SUBGRUPO,
		   ISNULL (NIVEIS.SUBGRUPO2, 'SEM CLASSIFICACAO') AS SUBGRUPO2,
		   3 AS NIVEL,
		   SALDOS.CONTA,
		...
		GROUP BY NIVEIS.CTS_CODPLA
			...
			, SALDOS.FILIAL
			, SALDOS.CC
			, NIVEIS.PAGTO_FIN_BANC
			, NIVEIS.EH_DEPRECIACAO
			, NIVEIS.INFORMATIVO
	)
	...
	SET @DESCRI_GRUPO = '03.(=)RECEITA BRUTA BASE ICMS';
	...
	SET @DESCRI_GRUPO = '05.(=)RECEITA LIQUIDA';
	INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
	...
	SET @DESCRI_GRUPO = '08.(=)SOBRA OPERACIONAL BRUTA';
	INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
	...
	BEGIN
		SET @DESCRI_GRUPO = '14.(=)SOBRA/PERDA OPERACIONAL';
		INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
		...
		SET @DESCRI_GRUPO = '17.(=)RESULT.EXERCICIO ACUMULADO';
		INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
