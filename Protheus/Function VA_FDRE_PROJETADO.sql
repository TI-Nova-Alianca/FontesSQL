...
BEGIN
	...
	;WITH CTS AS (
		SELECT CTS.CTS_CODPLA, CTS.CTS_VAGRP, RTRIM (CTS.CTS_DESCCG) + RTRIM (CTS_DETHCG) + CASE WHEN CTS_VASINF = 'S' THEN ' (informativo)' ELSE '' END AS SUBGRUPO
	...
	, NIVEIS AS (SELECT 1  AS ORDEM, '01.(+)INGRESSOS E RECEITAS'      AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 1
	   ...
	   UNION ALL SELECT 9  AS ORDEM, '09.(-)DESPESAS COM PESSOAL'      AS GRUPO, CTS.* FROM CTS WHERE CTS.CTS_VAGRP = 9
	...
	SELECT NIVEIS.CTS_CODPLA AS CODPLA,
		...
		GROUP BY NIVEIS.CTS_CODPLA, NIVEIS.ORDEM, NIVEIS.GRUPO, NIVEIS.SUBGRUPO, SALDOS.CONTA, CT1.CT1_DESC01, SALDOS.FILIAL, SALDOS.CC, NIVEIS.PAGTO_FIN_BANC, NIVEIS.EH_DEPRECIACAO, NIVEIS.INFORMATIVO
	)
