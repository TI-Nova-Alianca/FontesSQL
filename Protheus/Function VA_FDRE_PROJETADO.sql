...
(
	...
	returns @RET TABLE (CODPLA  VARCHAR (3)
		...
		unique NONCLUSTERED (CODPLA, ORDEM, GRUPO, SUBGRUPO, SUBGRUPO2, NIVEL, CONTA, FILIAL, CC)
		)
...
BEGIN
	...
	-- DADOS DA TABELA TABELA CTS (VISOES GERENCIAIS DA CONTABILIDADE) CONTENDO
	-- OS CAMPOS QUE VOU PRECISAR, JAH FILTRADO PELA VISAO QUE ME INTERESSA.
	;WITH SUB_CTS AS (
		SELECT CTS.CTS_CODPLA
			, CTS.CTS_VAGDRE
		--	, RTRIM (CTS.CTS_DESCCG) + RTRIM (CTS_DETHCG) + CASE WHEN CTS_VASINF = 'S' THEN ' (informativo)' ELSE '' END AS SUBGRUPO
		--	, RTRIM (CTS.CTS_DESCCG) + RTRIM (CTS_DETHCG) + CASE WHEN CTS_VASINF = 'S' THEN ' (informativo)' ELSE '' END AS SUBGRUPO2
			, CTS.CTS_NORMAL
			, CTS.CTS_CT1INI
			, CTS.CTS_CT1FIM
			, CTS.CTS_CTTINI
	...
	-- GERA UMA CTE CRIANDO COLUNAS DE ORDENACAO E DESCRICAO DOS GRUPOS PARA
	-- JUNTAR COM OS DADOS DA CTE ANTERIOR, DE FORMA A TER A ESTRUTURA EM UM
	-- FORMATO SEMELHENTE A TABELA CTS (VISOES GERENCIAIS) DA CONTABILIDADE,
	-- MAS JAH COM COLUNAS ADICIONAIS PARA AJUDAR A CALCULAR OS DADOS.
	-- AINDA TEM APENAS CONTA INICIAL E FINAL; NAO TEM CONTA A CONTA AINDA.
	, SUB_NIVEIS AS (
		          SELECT 1    AS ORDEM, '01. (+)INGRESSOS E RECEITAS'       AS GRUPO, '.'                           AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '01'
		UNION ALL SELECT 2    AS ORDEM, '02. (-)DEVOLUCOES'                 AS GRUPO, '.'                           AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '02'
--		UNION ALL SELECT 3    AS ORDEM, '03. (-)IMPOSTOS'                   AS GRUPO, '.'                           AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '03'
		UNION ALL SELECT 3.1  AS ORDEM, '03. (-)IMPOSTOS'                   AS GRUPO, 'COFINS'                      AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '03.1'
		UNION ALL SELECT 3.2  AS ORDEM, '03. (-)IMPOSTOS'                   AS GRUPO, 'ICMS'                        AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '03.2'
		UNION ALL SELECT 3.3  AS ORDEM, '03. (-)IMPOSTOS'                   AS GRUPO, 'IPI'                         AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '03.3'
		UNION ALL SELECT 3.4  AS ORDEM, '03. (-)IMPOSTOS'                   AS GRUPO, 'Icms ST'                     AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '03.4'
		UNION ALL SELECT 3.5  AS ORDEM, '03. (-)IMPOSTOS'                   AS GRUPO, 'PIS'                         AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '03.5'
		UNION ALL SELECT 4    AS ORDEM, '04. (=)RECEITA LIQUIDA'            AS GRUPO, '.'                           AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '04'
		UNION ALL SELECT 5    AS ORDEM, '05. (-)CPV'                        AS GRUPO, 'CUSTOS PRODUCAO'             AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '05.1'
		UNION ALL SELECT 5    AS ORDEM, '05. (-)CPV'                        AS GRUPO, 'GGF'                         AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '05.2'
		UNION ALL SELECT 5    AS ORDEM, '05. (-)CPV'                        AS GRUPO, 'MO'                          AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '05.3'
		UNION ALL SELECT 6    AS ORDEM, '06. (=)SOBRA OPERACIONAL BRUTA'    AS GRUPO, '.'                           AS SUBGRUPO, '.'                AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '06'
--		UNION ALL SELECT 7    AS ORDEM, '07. (-)DESPESAS OPERACIONAIS'      AS GRUPO, '.'                 AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE LIKE '07%'
		UNION ALL SELECT 7.1  AS ORDEM, '07. (-)DESPESAS OPERACIONAIS'      AS GRUPO, '(-)DESPESAS COM VENDAS'      AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '07.1'
--		UNION ALL SELECT 7.2  AS ORDEM, '07. (-)DESPESAS OPERACIONAIS'      AS GRUPO, '(-)DESPESAS COM PESSOAL'     AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '07.2'
		UNION ALL SELECT 7.21 AS ORDEM, '07. (-)DESPESAS OPERACIONAIS'      AS GRUPO, '(-)DESPESAS COM PESSOAL'     AS SUBGRUPO, '(-)PESSOAL ADMIN' AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '07.2.1'
		UNION ALL SELECT 7.22 AS ORDEM, '07. (-)DESPESAS OPERACIONAIS'      AS GRUPO, '(-)DESPESAS COM PESSOAL'     AS SUBGRUPO, '(-)PESSOAL COML'  AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '07.2.2'
		UNION ALL SELECT 7.3  AS ORDEM, '07. (-)DESPESAS OPERACIONAIS'      AS GRUPO, '(-)DESPESAS ADMINISTRATIVAS' AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '07.3'
		UNION ALL SELECT 7.4  AS ORDEM, '07. (-)DESPESAS OPERACIONAIS'      AS GRUPO, '(-)DESPESAS TRIBUTARIAS'     AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '07.4'
		UNION ALL SELECT 8    AS ORDEM, '08. SOBRA/PERDA OPERACIONAL'       AS GRUPO, '.'                 AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '08'
		UNION ALL SELECT 9    AS ORDEM, '09. RESULTADO FINANCEIRO'          AS GRUPO, '.'                 AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '09'
		UNION ALL SELECT 10   AS ORDEM, '10. RESULTADO NAO OPERACIONAL'     AS GRUPO, '.'                 AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '10'
		UNION ALL SELECT 99   AS ORDEM, '99. CONTAS NAO CLASSIFICADAS'      AS GRUPO, '.'                 AS SUBGRUPO, '.'      AS SUBGRUPO2, SUB_CTS.* FROM SUB_CTS WHERE SUB_CTS.CTS_VAGDRE = '99'
	)
	...
	,SUB_FINAL AS (
	SELECT
		SUB_NIVEIS.CTS_CODPLA AS CODPLA,
		ISNULL (SUB_NIVEIS.ORDEM, 99) AS ORDEM,
		SUB_NIVEIS.GRUPO,    --ISNULL (SUB_NIVEIS.GRUPO,    'ANTIGO 18.INFORMATIVOS') AS GRUPO,
		SUB_NIVEIS.SUBGRUPO, --ISNULL (SUB_NIVEIS.SUBGRUPO, 'SEM CLASSIFICACAO') AS SUBGRUPO,
		SUB_NIVEIS.SUBGRUPO2,--ISNULL (SUB_NIVEIS.SUBGRUPO2, 'SEM CLASSIFICACAO') AS SUBGRUPO2,
		3 AS NIVEL,
		SUB_SALDOS.CONTA,
		LTRIM (RTRIM (CT1.CT1_DESC01)) AS CT1_DESC01,
		SUB_SALDOS.CC,
		SUB_SALDOS.FILIAL,
		SUM (ORCADO_ANO       * CASE SUB_NIVEIS.CTS_NORMAL WHEN '1' THEN 1 ELSE -1 END) AS ORC_ANO,
		'F' AS ORC_ANO_FP,
		0 AS ORC_ANO_AV,
		SUM (ORCADO           * CASE SUB_NIVEIS.CTS_NORMAL WHEN '1' THEN 1 ELSE -1 END) AS ORC_PER,
		'F' AS ORC_PER_FP,
		0 AS ORC_PER_AV,
		SUM (REA_MES          * CASE SUB_NIVEIS.CTS_NORMAL WHEN '1' THEN 1 ELSE -1 END * CASE WHEN SUB_NIVEIS.ORDEM IN (2,4,6) THEN -1 ELSE 1 END) AS REA_MES,
		'F' AS REA_MES_FP,
		0 AS REA_MES_AV,
		SUM (REALIZADO        * CASE SUB_NIVEIS.CTS_NORMAL WHEN '1' THEN 1 ELSE -1 END * CASE WHEN SUB_NIVEIS.ORDEM IN (2,4,6) THEN -1 ELSE 1 END) AS REA_PER,
		'F' AS REA_PER_FP,
		0 AS REA_PER_AV,
		SUM (REALIZADO_ANOANT * CASE SUB_NIVEIS.CTS_NORMAL WHEN '1' THEN 1 ELSE -1 END * CASE WHEN SUB_NIVEIS.ORDEM IN (2,4,6) THEN -1 ELSE 1 END) AS REA_ANT,
		'F' AS REA_ANT_FP,
		0 AS REA_ANT_AV,
		...
		FROM CT1010 CT1
			,SUB_SALDOS
			LEFT JOIN SUB_NIVEIS
				ON (SUB_SALDOS.CONTA BETWEEN SUB_NIVEIS.CTS_CT1INI AND SUB_NIVEIS.CTS_CT1FIM
				AND SUB_SALDOS.CC    BETWEEN SUB_NIVEIS.CTS_CTTINI AND SUB_NIVEIS.CTS_CTTFIM)
		WHERE CT1.D_E_L_E_T_ = ''
		AND CT1.CT1_FILIAL = '  '
		AND CT1.CT1_CONTA = SUB_SALDOS.CONTA
		AND (SUB_SALDOS.ORCADO_ANO  != 0
	...
	WHERE EXISTS (SELECT * FROM @LIBERACOES L WHERE CC LIKE L.LIB_CC AND 'GRUPO' + CAST (ORDEM AS VARCHAR (4)) = L.LIB_CONTA)
	--
	...
	INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
	VALUES           (@VISAO_PRINCIPAL, 3,     @DESCRI_GRUPO, @DESCRI_GRUPO, 1,     'N',      '.')
	...
	INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
	VALUES           (@VISAO_PRINCIPAL, 5,     @DESCRI_GRUPO, @DESCRI_GRUPO, 1,     'S',      '.')
	...
	INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
	VALUES           (@VISAO_PRINCIPAL, 8,     @DESCRI_GRUPO, @DESCRI_GRUPO, 1,     'N',      '.')
	...
	BEGIN
		...
		INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
		VALUES           (@VISAO_PRINCIPAL, 14,    @DESCRI_GRUPO, @DESCRI_GRUPO, 1,    'S',       '.')
		...
		INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,      NIVEL, DESTACAR, CONTA)
		VALUES           (@VISAO_PRINCIPAL, 17,    @DESCRI_GRUPO, @DESCRI_GRUPO, 1,    'S',       '.')
	...
	INSERT INTO @RET (CODPLA, ORDEM, GRUPO, SUBGRUPO, NIVEL,
		...
		SELECT DISTINCT @VISAO_PRINCIPAL, ORDEM, GRUPO, SUBGRUPO, 2,
						 SUM (ORC_ANO   ) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 MAX (ORC_ANO_FP) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TER UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						 SUM (ORC_ANO_AV) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 SUM (ORC_PER   ) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 MAX (ORC_PER_FP) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TER UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						 SUM (ORC_PER_AV) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 SUM (REA_MES   ) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 MAX (REA_MES_FP) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TER UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						 SUM (REA_MES_AV) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 SUM (REA_PER   ) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 MAX (REA_PER_FP) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TER UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						 SUM (REA_PER_AV) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 SUM (REA_ANT   ) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),
						 MAX (REA_ANT_FP) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TER UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						 SUM (REA_ANT_AV) OVER (PARTITION BY ORDEM, GRUPO, SUBGRUPO)
	...
	INSERT INTO @RET (CODPLA, ORDEM, GRUPO, NIVEL,
					 ORC_ANO, ORC_ANO_FP, ORC_ANO_AV,
					 ORC_PER, ORC_PER_FP, ORC_PER_AV,
					 REA_MES, REA_MES_FP, REA_MES_AV,
					 REA_PER, REA_PER_FP, REA_PER_AV,
					 REA_ANT, REA_ANT_FP, REA_ANT_AV)
		SELECT DISTINCT @VISAO_PRINCIPAL, ORDEM, GRUPO, 1,
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE ORC_ANO    END) OVER (PARTITION BY ORDEM, GRUPO),
						MAX (ORC_ANO_FP) OVER (PARTITION BY ORDEM, GRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TEM UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE ORC_ANO_AV END) OVER (PARTITION BY ORDEM, GRUPO),
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE ORC_PER    END) OVER (PARTITION BY ORDEM, GRUPO),
						MAX (ORC_PER_FP) OVER (PARTITION BY ORDEM, GRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TEM UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE ORC_PER_AV END) OVER (PARTITION BY ORDEM, GRUPO),
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE REA_MES    END) OVER (PARTITION BY ORDEM, GRUPO),
						MAX (REA_MES_FP) OVER (PARTITION BY ORDEM, GRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TEM UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE REA_MES_AV END) OVER (PARTITION BY ORDEM, GRUPO),
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE REA_PER    END) OVER (PARTITION BY ORDEM, GRUPO),
						MAX (REA_PER_FP) OVER (PARTITION BY ORDEM, GRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TEM UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE REA_PER_AV END) OVER (PARTITION BY ORDEM, GRUPO),
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE REA_ANT    END) OVER (PARTITION BY ORDEM, GRUPO),
						MAX (REA_ANT_FP) OVER (PARTITION BY ORDEM, GRUPO),  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TEM UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
						SUM (CASE WHEN INFORMATIVO = 'S' THEN 0 ELSE REA_ANT_AV END) OVER (PARTITION BY ORDEM, GRUPO)
		FROM @RET WHERE CODPLA = @VISAO_PRINCIPAL AND NIVEL = 3
		AND NIVEL != 1  -- AS LINHAS DE TOTAIS SAO UNICAS E NAO HA NECESSIDADE DE CALCULAR NIVEIS PARA ELAS.

	-- MARCA A PRIMEIRA LINHA PARA SER DESTACADA, PARA EVIDENCIAR O FATURAMENTO
...
/* TESTES PARA QUERYVIEW DO GX
	-- MUDA A DESCRICAO DO SUBGRUPO DE DESPESAS COM PESSOAL, 'FAZENDO DE CONTA' QUE TRATA-SE DE UM NOVO SUBNIVEL.
	UPDATE @RET SET SUBGRUPO = 'DESPESAS COM PESSOAL'
		WHERE ORDEM = 9
		AND SUBGRUPO2 IN ('PESSOAL ADMIN', 'PESSOAL COML')

	-- INSERE NOVO NIVEL PARA TESTES
		UPDATE @RET SET SUBGRUPO2 = SUBGRUPO
		SET @DESCRI_GRUPO = '01.(+)INGRESSOS E RECEITAS';
		INSERT INTO @RET (CODPLA,           ORDEM, GRUPO,         SUBGRUPO,               SUBGRUPO2, NIVEL, DESTACAR, CONTA)
		VALUES           (@VISAO_PRINCIPAL, 1,     @DESCRI_GRUPO, 'INGRESSOS E RECEITAS', 'TESTE',   4,    'S',       '.')
		UPDATE @RET SET ORC_ANO =    (SELECT SUM (ORC_ANO)    FROM @RET WHERE ORDEM IN (14,15,16) AND INFORMATIVO != 'S')
					   ,ORC_ANO_FP = (SELECT MAX (ORC_ANO_FP) FROM @RET WHERE ORDEM IN (14,15,16) AND INFORMATIVO != 'S')  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TEM UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
					   ,ORC_PER =    (SELECT SUM (ORC_PER)    FROM @RET WHERE ORDEM IN (14,15,16) AND INFORMATIVO != 'S')
					   ,ORC_PER_FP = (SELECT MAX (ORC_PER_FP) FROM @RET WHERE ORDEM IN (14,15,16) AND INFORMATIVO != 'S')  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TEM UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
					   ...
					   ,REA_ANT =    (SELECT SUM (REA_ANT)    FROM @RET WHERE ORDEM IN (14,15,16) AND INFORMATIVO != 'S')
					   ,REA_ANT_FP = (SELECT MAX (REA_ANT_FP) FROM @RET WHERE ORDEM IN (14,15,16) AND INFORMATIVO != 'S')  -- CONSEGUI USAR 'MAX' AQUI POR QUE SEMPRE TEM [FP], E P EH MAIOR QUE F, E BASTA TEM UM P EM QQUER LINHA PARA QUE ASSUMA P NO NIVEL ACIMA.
				WHERE CODPLA = @VISAO_PRINCIPAL AND ORDEM = 18
*/
