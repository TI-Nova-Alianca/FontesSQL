...
/****** Object:  Job [Bkp_full_BI_Alianca]    Script Date: 17/03/2022 11:39:39 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 17/03/2022 11:39:39 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Bkp_full_BI_Alianca', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Bkp_full_BIAlianca]    Script Date: 17/03/2022 11:39:39 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Bkp_full_BIAlianca', 
		@step_id=1, 
...
declare @nome_banco varchar (max) set @nome_banco = ''BI_Alianca'';
...
declare @destino varchar (max) set @destino = ''\\192.168.1.104\bkp-SQL\BI_Alianca\''; 
...
set @sql = ''BACKUP DATABASE '' +(select upper(@nome_banco))+ '' TO DISK = '''''' + @name_file + ''.bak''''''  + '' WITH COMPRESSION, INIT''
print @sql
exec (@sql);
', 
