...
/****** Object:  Job [Bkp_log_naweb]    Script Date: 17/03/2022 11:39:04 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 17/03/2022 11:39:04 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Bkp_log_naweb', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Bkp_log_naweb]    Script Date: 17/03/2022 11:39:04 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Bkp_log_naweb', 
		@step_id=1, 
...
declare @nome_banco varchar (50) set @nome_banco = ''Naweb'';
...
set @sql = '''';
set @sql = @sql + '' BACKUP LOG ''+(select lower (@nome_banco));
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Diario a Cada 1Hr as 01:30', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=8, 
		...
		@schedule_uid=N'ecd81038-4264-48b8-bcd7-0ff58fbb9bf2'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
